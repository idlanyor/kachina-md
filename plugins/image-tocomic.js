        ...
        try {
            ...
            if (!hasArgUrl) {
                ...
                if (!allowed.includes(refType)) {
                    await m.reply('❌ Reply/kirim gambar (atau stiker) dengan caption .tocomic, atau berikan URL gambar')
                    return
                }
            ...
            await sock.sendMessage(m.chat, {
                ...
                caption: '🎭 Converted to comic style'
            }, { quoted: m })

            await sock.sendMessage(m.chat, { react: { text: '✅', key: m.key } })

        } catch (error) {
            console.error('Error in tocomic:', error)
            await sock.sendMessage(m.chat, { react: { text: '❌', key: m.key } })
            await m.reply('❌ Gagal convert ke comic: ' + (error?.message || 'Unknown error'))
        }
    }
}
